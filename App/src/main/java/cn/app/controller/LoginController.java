...
public class LoginController {
	...
	public String login2_post(String userCode,
		...
		if(null != bUser){//登录成功
			...
			model.addAttribute("userCode", bUser.getUsercode());
			model.addAttribute("userName", bUser.getUsername());
...
System.out.println(devCode+"==="+devPassword);
		...
		if(null != duser){//登录成功
			...
			model.addAttribute("userCode", duser.getDevcode());
			model.addAttribute("userName", duser.getDevname());
